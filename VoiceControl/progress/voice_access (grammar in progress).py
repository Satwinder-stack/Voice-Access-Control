...
def autocorrect_and_punctuate(text):
    if not text.strip():
        return text

    # Step 1: Autocorrect common typos
    words = text.split()
    corrected_words = [AUTOCORRECT.get(word.lower(), word) for word in words]
    ...
    # Step 3: Smart comma placement
    # Only add comma before "and/but/so/etc." if it connects two full clauses
    # (i.e., there's a subject-verb on both sides — simple heuristic)
    conj_pattern = r'(\b(and|but|so|or|nor|for|yet)\b)(\s+[a-z]+\s+[a-z]+)'
    text = re.sub(conj_pattern, r', \1\3', text, flags=re.IGNORECASE)

    # Add comma after common introductory phrases (only if followed by a clause)
    intro_words = r'\b(however|meanwhile|therefore|also|finally|first|next|then|after|before|although|because|since|while|if|when|though|even though)\b'
    text = re.sub(intro_words + r'(\s+[a-z])', r'\1,\2', text, flags=re.IGNORECASE)
...
- delete [number] times → backspace
    ...
    if writing_match:
        ...
        # Spelling mode detection
        if len(words) > 3 and all(len(w.strip(".,!?")) <= 2 for w in words):
            text_to_type = raw_text.replace("-", "").replace(" ", "")
        else:
            clean_text = raw_text.replace("-", " ")
            text_to_type = autocorrect_and_punctuate(clean_text)
    ...
    # === ADD SYMBOLS ===
        # === ADD SYMBOLS (improved reliability) ===
    if cmd.startswith("add "):
        symbol_phrase = cmd[4:].strip()
        symbols = {
        ...
        if symbol_phrase in symbols:
            time.sleep(0.1)  # tiny delay for focus
            pyautogui.typewrite(symbols[symbol_phrase])
            speak(f"{symbol_phrase.title()} added")
            return
